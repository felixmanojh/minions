...
if [ -t 0 ] && [ -z "$MINIONS_PRESET" ]; then
    ...
    echo ""
    echo "  medium - ~13GB download, needs 16GB RAM (recommended) [default]"
    echo "           Qwen2.5-Coder:7B (Implementer, code generation)"
    echo "           DeepSeek-Coder:6.7B (Reviewer, bug detection)"
    echo "           StarCoder2:7B (Patcher, surgical edits and FIM)"
    echo ""
    echo "  large  - ~35GB download, needs 32GB+ RAM"
    echo "           Best for large repo refactoring and deeper reasoning"
    echo ""
    echo "  lite   - ~5GB download, needs 8GB RAM"
    echo "           Single strong generalist model for smaller projects"
    echo ""
...
echo ""
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}  Minions ready! ($PRESET preset)${NC}"
echo -e "${GREEN}========================================${NC}"
echo ""
echo "Minions auto assigns the right model per task:"
echo "  • Implementer → code generation and refactors"
echo "  • Reviewer    → correctness, bugs, and safety checks"
echo "  • Patcher     → surgical edits and FIM"
echo ""
echo "Next steps:"
...
echo ""
echo "Works inside any repo folder."
echo "To override models, edit: llm_gc/config/models.yaml"
echo ""
