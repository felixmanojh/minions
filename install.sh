...
if [ -t 0 ] && [ -z "$MINIONS_PRESET" ]; then
    ...
    echo ""
    echo "  medium - ~13GB download, 16GB RAM (recommended) [default]"
    echo "           Qwen2.5-Coder:7B  (implementer)"
    echo "           DeepSeek-Coder:6.7B (reviewer)"
    echo "           StarCoder2:7B (surgical edits)"
    echo ""
    echo "  large  - ~35GB download, 32GB+ RAM"
    echo "           Best for large repo refactoring"
    echo ""
    echo "  lite   - ~5GB download, 8GB RAM"
    echo "           Single strong generalist model"
    echo ""
...
echo "  • Implementer → code generation"
echo "  • Reviewer    → bug detection"
echo "  • Patcher     → surgical edits (FIM)"
echo ""
echo "Next steps:"
...
echo ""
echo "Override models: llm_gc/config/models.yaml"
echo ""
